...
namespace Atlas.Application.CQRS.Categories.Queries.SearchCategoriesByGoodName
{
        ...
        SearchedCategoryListVm>
    {
        ...
        public async Task<SearchedCategoryListVm> Handle(SearchCategoriesByGoodNameQuery request, CancellationToken cancellationToken)
        {
            ...
            var goodIds = await _dbContext.Goods.Where(x => x.IsDeleted == false).OrderByDescending(x => EF.Functions.TrigramsSimilarity(
            ...
            try
            {
                var cursor = await session.RunAsync("MATCH (c:Category)<-[r:BELONGS_TO]-(g:Good) WHERE g.Id IN $Ids RETURN {Id: c.Id, Name: c.Name, NameRu: c.NameRu, NameEn: c.NameEn, NameUz: c.NameUz, Count: COUNT(r), OrderNumber: c.OrderNumber, IsHidden: c.IsHidden, IsVerified: c.IsVerified};", new
                {
                    Ids = goodIds
                });
